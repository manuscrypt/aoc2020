...
targetBag : Bag
targetBag =
    { mod = "shiny", color = "gold" }


containsTarget : Bag -> Container -> Bool
containsTarget bag cont =
    cont.contains |> List.any (Tuple.second >> (==) bag)


find : List Container -> List Bag -> List Bag -> List Bag
find list targets results =
    let
        found =
            targets
                |> List.foldl
                    (\target acc ->
                        list
                            |> List.filter (containsTarget target)
                            |> List.append acc
                    )
                    []
    in
    if List.length found > 0 then
        let
            bags =
                List.map .bag found
        in
        find list bags (results ++ bags)

    else
        results


count : List Container -> List Bag -> Int -> Int
count list targets result =
    let
        found =
            targets
                |> List.foldl
...
main =
    let
        partA =
            find (containers input) [ targetBag ] []
                |> List.uniqueBy (\c -> c.color ++ c.mod)
                |> List.length

        partB =
            count (containers input) [ targetBag ] 0
    in
    div []
        [ output "part1"
...
containers : String -> List Container
containers allLines =
    allLines
        |> String.lines
        |> List.filterMap
            (\line ->
                case Parser.run parseContainer line of
                    Ok data ->
                        Just data
...
parseContainer : Parser Container
parseContainer =
    succeed Container
        |= parseBag
        |. Parser.spaces
        ...
        |= oneOf
            [ succeed identity
                |= Parser.sequence { start = "", separator = ",", end = ".", spaces = Parser.spaces, item = item, trailing = Parser.Forbidden }
            , succeed identity
                |. Parser.chompUntil "."
                |= Parser.succeed []
            ]
...
parseBag =
    succeed Bag
        ...
        |= (Parser.chompUntil " " |> Parser.getChompedString)
        |. Parser.spaces
        |. oneOf [ keyword "bag", keyword "bags" ]
