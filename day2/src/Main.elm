...
isValidPartA : Input -> Bool
isValidPartA inp =
    let
        c =
            String.toList inp.pw
                |> List.filter (\d -> String.fromChar d == inp.c)
                |> List.length
    in
    (c >= inp.min) && (c <= inp.max)


isValidPartB : Input -> Bool
isValidPartB inp =
    let
        arr =
            inp.pw
        ...
        getAt idx =
            Array.get (idx - 1) arr |> Maybe.map String.fromChar
    in
    List.filterMap identity
        [ getAt inp.min
        , getAt inp.max
        ]
        |> List.filter (\x -> x == inp.c)
        |> List.length
        |> (==) 1
    ...
    case String.words s of
        [ a, b, c ] ->
            case String.split "-" a of
                [ min, max ] ->
                    Just <|
                        Input (String.toInt min |> Maybe.withDefault 0)
                            (String.toInt max |> Maybe.withDefault 0)
                            (String.slice 0 1 b)
                            c
